...
YOUR_CHANNEL  = "@StudySnapUpdates"
BOT_USERNAME  = "@StudyBot"
FREE_LIMIT    = 10
...
def add_paid_user(user_id, extra_days=0):
    # If already paid, extend; otherwise start fresh from now
    conn = db()
    cur  = conn.cursor()
    cur.execute("SELECT expires_at FROM paid_users WHERE user_id = ?", (str(user_id),))
    row  = cur.fetchone()
...
def record_referral(referrer_id, referred_id):
    """Save the referral only if referred_id is new."""
    conn = db()
    cur  = conn.cursor()
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle referral link: /start ref1234567
    if ctx.args:
        arg = ctx.args[0]
    ...
    await update.message.reply_text(
        ...
        "⭐ Pro: رسائل غير محدودة مقابل 100 Star/شهر\n\n"
...
async def handle(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Answer clearly, briefly, and in the same language the user writes in. "
                        "If they write in Arabic, reply in Arabic. If English, reply in English."
                    ),
                },
        ...
        await update.message.reply_text(
            "⚠️ حدث خطأ، حاول مرة أخرى بعد لحظة."
        )
        print(f"Groq error: {e}")


# ── Daily auto-promo to your channel ─────────────────────────────────────────
...
# Schedule daily promo at 18:00 (6pm)
